...
export default function CampaignDetailModal({ visible, onClose, campaign, onCustomerAction }: CampaignDetailModalProps) {
  ...
  const hasCustomerAction = campaign.customer_action_enabled && Boolean(onCustomerAction) && campaign.is_purchasable !== true;
      ...
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
        <View style={{ backgroundColor: Colors.surface, borderTopLeftRadius: ss(24), borderTopRightRadius: ss(24), padding: ss(24), maxHeight: '86%' }}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {hasCustomerAction ? (
              <AppButton
                label={campaign.cta_label || t('campaign.viewDetails', 'View Details')}
                onPress={() => onCustomerAction?.(campaign)}
              />
            ) : (
